...
test('readTranscriptUsage counts usage from a repeated message.id only once', async () => {
  // Claude Code writes one line per content block (thinking/text/tool_use) of a
  // single API response, repeating the same usage on each line under the same id.
  ...
  const usage = await readTranscriptUsage('/t.jsonl', { read: async () => lines });
  assert.deepEqual(usage, { inputTokens: 2, outputTokens: 138, cacheCreationInputTokens: 12942, cacheReadInputTokens: 19608 });
});
...
test('readTranscriptUsage sums usage from distinct message.id values', async () => {
  ...
  const usage = await readTranscriptUsage('/t.jsonl', { read: async () => lines });
  assert.deepEqual(usage, { inputTokens: 3, outputTokens: 3, cacheCreationInputTokens: 3, cacheReadInputTokens: 3 });
});
...
test('readTranscriptUsage handles usage objects with missing token properties', async () => {
  ...
  const usage = await readTranscriptUsage('/t.jsonl', { read: async () => lines });
  assert.deepEqual(usage, { inputTokens: 1, outputTokens: 0, cacheCreationInputTokens: 0, cacheReadInputTokens: 0 });
});
...
test('readTranscriptUsage correctly handles sidechain flag in assistant entries', async () => {
  ...
  const usage = await readTranscriptUsage('/t.jsonl', { read: async () => lines });
  assert.deepEqual(usage, { inputTokens: 5, outputTokens: 10, cacheCreationInputTokens: 15, cacheReadInputTokens: 20 });
});
...
test('readTranscriptUsage accumulates multiple entries correctly', async () => {
  ...
  const usage = await readTranscriptUsage('/t.jsonl', { read: async () => lines });
  assert.deepEqual(usage, { inputTokens: 2, outputTokens: 3, cacheCreationInputTokens: 4, cacheReadInputTokens: 5 });
});
